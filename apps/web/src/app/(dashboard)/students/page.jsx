...
// Date helpers for DOB constraints
const today      = new Date().toISOString().split('T')[0];
const minDobDate = '1990-01-01'; // earliest sensible birth year
...
const columns = (onView, onWithdraw) => [
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="text-sm">{row.original.dateOfBirth ? formatDate(row.original.dateOfBirth) : '—'}</span>,
  },
  {
    accessorKey: 'enrollmentDate',
    header: 'Enrolled',
    cell: ({ row }) => <span className="text-sm">{row.original.enrollmentDate ? formatDate(row.original.enrollmentDate) : '—'}</span>,
  },
  {
    accessorKey: 'status',
    header: 'Status',
...
export default function StudentsPage() {
  ...
  const { mutate: createStudent, isPending } = useMutation({
    mutationFn: (formData) => {
      // Build the payload — only include guardian if any guardian field is filled
      const {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <Select value={selectedStatus} onValueChange={(v) => setSelectedStatus(v === '__all__' ? '' : v)}>
          ...
          <form onSubmit={handleSubmit(createStudent)} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                {/* min/max prevents the browser from restricting year navigation */}
                <Input
                  type="date"
