    ...
    <>
      ...
      <div className="max-w-[780px] mx-auto p-6 bg-white" id="report-print-root">
        ...
        <div className="text-center border-b-2 border-black pb-3 mb-4">
          <h1 className="text-2xl font-bold uppercase tracking-wide">{school?.name ?? 'School Name'}</h1>
          {school?.address && <p className="text-sm">{school.address}{school.county ? `, ${school.county}` : ''}</p>}
          {school?.phone && <p className="text-sm">Tel: {school.phone}</p>}
          {school?.email && <p className="text-sm">{school.email}</p>}
          {schoolMotto && <p className="text-sm italic mt-1">"{schoolMotto}"</p>}
          <h2 className="text-lg font-bold mt-2 uppercase tracking-widest">
        ...
        <div className="mt-6 pt-3 border-t border-gray-300 text-center text-xs text-gray-400">
          Generated by EduSaaS · {school?.name ?? ''} · {rc.academicYear} {rc.term}
        </div>
      </div>
    </>
  );
