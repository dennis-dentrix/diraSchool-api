  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <InfoItem
              label="Full Name"
              value={typeof student === 'object' ? `${student.firstName} ${student.lastName}` : '—'}
            />
            <InfoItem label="Admission No." value={typeof student === 'object' ? student.admissionNumber : '—'} />
            <InfoItem label="Class" value={cls ? `${cls.name}${cls.stream ? ` ${cls.stream}` : ''}` : '—'} />
              ...
              <thead>
                <tr className="border-b bg-muted/30">
                  ...
                  <th className="text-left py-2.5 px-4 font-medium text-muted-foreground">
                    Teacher Remark
                    {isDraft && <span className="text-xs font-normal ml-1 text-muted-foreground">(editable)</span>}
                  </th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={subKey} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="py-3 px-3 text-center">
                        {subject.grade ? (
                          <Badge className={`text-xs ${gradeColors[subject.grade] ?? 'bg-gray-100 text-gray-800'}`}>
                            {subject.grade}
                          </Badge>
                        ) : '—'}
                      </td>
              ...
              <tfoot>
                <tr className="border-t bg-muted/30">
                  ...
                  <td className="py-2.5 px-3 text-center">
                    {rc.overallGrade ? (
                      <Badge className={`${gradeColors[rc.overallGrade] ?? 'bg-gray-100 text-gray-800'}`}>
                        {rc.overallGrade}
                      </Badge>
                    ) : '—'}
                  </td>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {editing ? (
            <>
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="outline" onClick={() => {
                  setEditing(false);
                  setRemarks({ teacherRemarks: rc.teacherRemarks ?? '', principalRemarks: rc.principalRemarks ?? '' });
                }}>
                  Cancel
                </Button>
              </div>
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs text-muted-foreground mb-1">Class Teacher's Remarks</p>
                <p className="text-sm">{rc.teacherRemarks || <span className="text-muted-foreground italic">No remarks added.</span>}</p>
              </div>
              <Separator />
              <div>
                <p className="text-xs text-muted-foreground mb-1">Principal's Remarks</p>
                <p className="text-sm">{rc.principalRemarks || <span className="text-muted-foreground italic">No remarks added.</span>}</p>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {(rc.subjects ?? []).map((subject) => (
            <div key={subject.subjectId?.toString() ?? subject.subjectName} className="border-b last:border-0">
                  ...
                  <tbody className="divide-y divide-border/50">
                    {(subject.exams ?? []).map((exam) => (
                      <tr key={exam.examId?.toString()} className="hover:bg-muted/10">
                        ...
                        <td className="px-3 py-1.5 capitalize text-muted-foreground">{exam.examType}</td>
                        <td className="px-3 py-1.5 text-center tabular-nums">{exam.marks}</td>
                        <td className="px-3 py-1.5 text-center tabular-nums text-muted-foreground">{exam.totalMarks}</td>
                        <td className="px-3 py-1.5 text-center tabular-nums">{exam.percentage?.toFixed(1)}%</td>
                        <td className="px-3 py-1.5 text-center">
                          {exam.grade ? (
                            <Badge className={`text-xs ${gradeColors[exam.grade] ?? 'bg-gray-100 text-gray-800'}`}>
                              {exam.grade}
                            </Badge>
                          ) : '—'}
                        </td>
