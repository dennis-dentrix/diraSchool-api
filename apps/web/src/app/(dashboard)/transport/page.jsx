...
import { Plus, Bus, MoreHorizontal, Trash2 } from 'lucide-react';
...
export default function TransportPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['transport-routes'], queryFn: async () => { const res = await transportApi.listRoutes({ limit: 50 }); return res.data; } });

  const { mutate: createRoute, isPending } = useMutation({
    mutationFn: (data) => transportApi.createRoute(data),
    onSuccess: () => { toast.success('Route created'); queryClient.invalidateQueries({ queryKey: ['transport-routes'] }); setOpen(false); reset(); },
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-40" />)}</div>
      ) : routes.length === 0 ? (
        <EmptyState icon={Bus} title="No routes configured" description="Add transport routes for student pickup and drop-off" action={{ label: 'Add Route', onClick: () => setOpen(true) }} />
      ) : (
                ...
                <div className="flex items-start justify-between">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem className="text-destructive" onClick={() => { if (confirm('Delete route?')) deleteRoute(route._id); }}>Delete</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              </CardHeader>
